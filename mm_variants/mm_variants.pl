...
split_categories([], [[]]).
split_categories([H|T], [[H]|SplitT]) :-
	split_categories_aux(T, SplitT).
...
write_variants([v(Word,Categories,VarLevel,History,Roots,_NFR)|Rest],
    ...
    format('~a|~p|~a|~p|~d|~s|~a|~p~n',
	   [Term,SimplifiedTermCats,Word,SimplifiedCats,VarLevel,History,FirstOfWord,Roots]),
    write_variants(Rest, Term, SimplifiedTermCats).
